...
#Initial state
flu_zero <- c(S_A = 999, E_A = 0, I_A = 1, R_A = 0,
              S_T = 999, E_T = 0, I_T = 1, R_T = 0)
...
seirv <- function(t, y, parms){
  with(c(as.list(y), parms), {
    ...
    # risk averse
    dS_Adt <- -(lambda_AA + lambda_TA)*S_A * I/N
    dE_Adt <- (lambda_AA + lambda_TA) * S_A * I/N - (gamma) * E_A
    # dS_Adt <- (b*N_A + tau * V_A) - (mu + nu + FOI_AA + FOI_TA)*S_A
    # dE_Adt <- (FOI_AA + FOI_TA) * S_A - (gamma + mu) * E_A
    dI_Adt <- gamma * E_A - (delta + beta_AA + beta_AT + epsilon) * I_A
    dR_Adt <- delta * I_A
    #dV_Adt <- nu * S_A - (tau + mu) * V_A
    #risk tolerant
    dS_Tdt <- - (lambda_TT + lambda_AT)*S_T * I/N
    dE_Tdt <- (lambda_TT + lambda_AT) * S_T * I/N - (gamma) * E_T
    # dS_Tdt <- (b*N_T + tau * V_T) - (mu + nu + FOI_TT + FOI_AT)*S_T
    # dE_Tdt <- (FOI_TT + FOI_AT) * S_T - (gamma + mu) * E_T
    dI_Tdt <- gamma * E_T - (delta + beta_TT + beta_TA + epsilon) * I_T
    dR_Tdt <- delta * I_T
    #dV_Tdt <- nu * S_T - (tau + mu) * V_T
    return(list(c(dS_Adt, dE_Adt, dI_Adt, dR_Adt, dS_Tdt, dE_Tdt, dI_Tdt, dR_Tdt)))
  })
}
